...
export async function buildSyncPayload() {
  const today = await db.dayPlans.get(todayKey()); const tomorrow = await db.dayPlans.get(tomorrowKey())
  const taskIds = new Set([...(today?.taskIds ?? []), ...(tomorrow?.taskIds ?? [])])
  ...
  return { schemaVersion: 1, clientId: 'sera-dayboard-tablet', sentAt: new Date().toISOString(), today, tomorrow, tasks, captures, interruptions, resets, endOfDayNote: today?.endOfDayNote ?? '' }
}
...
export async function syncHome(settings: SyncSettings) {
  ...
  try {
    ...
    const result = await response.json() as { syncedAt?: string; dayPlan?: DayPlan; tasks?: Task[] }
