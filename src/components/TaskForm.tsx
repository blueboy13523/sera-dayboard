...
export type TaskFormData = { title: string; category: string; priority: Priority; estimatedMinutes: number; plannedStart?: string; notes?: string; majorObjective?: boolean }
export function TaskForm({ task, submitLabel = 'Add to today', onSubmit, onCancel }: { task?: Task; submitLabel?: string; onSubmit: (d: TaskFormData) => void; onCancel: () => void }) {
  const [data, setData] = useState<TaskFormData>({ title: task?.title ?? '', category: task?.category ?? 'Personal', priority: task?.priority ?? 'medium', estimatedMinutes: task?.estimatedMinutes ?? 30, plannedStart: task?.plannedStart ?? '', notes: task?.notes ?? '', majorObjective: task?.majorObjective ?? false })
  return <form onSubmit={e => { e.preventDefault(); if (data.title.trim()) onSubmit({ ...data, title: data.title.trim() }) }}>
    ...
    <label className="major-toggle"><input type="checkbox" checked={data.majorObjective} onChange={e => setData({ ...data, majorObjective: e.target.checked })} /><span><strong>Major Objective</strong><small>Use a slightly bigger celebration when this is completed.</small></span></label>
