...
export function ResetExperience({ task, fromInterruption, initialDuration, onCancel, onResume }: { task: Task; fromInterruption: boolean; initialDuration?: ResetDuration; onCancel: () => void; onResume: () => void | Promise<void> }) {
  ...
  useEffect(() => { getResetPreferences().then(saved => setPrefs({ ...saved, preferredDuration: initialDuration ?? saved.preferredDuration })) }, [initialDuration])
  ...
  const begin = async () => {
    beginAudio(prefs.audioChoice, prefs.ambientVolume)
    await saveResetPreferences(prefs)
    const session = await startResetSession(task.id, prefs.preferredDuration)
  ...
  return <div className={`reset-screen reset-${stage}`} role="dialog" aria-modal="true" aria-label="Guided reset">
    {stage === 'setup' && <div className="reset-setup"><button className="reset-close" onClick={onCancel} aria-label="Close reset"><X /></button><p className="reset-kicker">RESET</p><h1>Take a little space.</h1><p className="reset-lead">Breathe in for 4. Breathe out for 6. No holding, no pressure.</p>
      ...
      <button className="button reset-begin" onClick={begin}><Play fill="currentColor" /> Begin {prefs.preferredDuration}-minute reset</button></div>}
    ...
    {stage === 'return' && <div className="reset-return"><div className="return-icon">✓</div><p className="reset-kicker">RESET COMPLETE</p><h1>Come back to just this.</h1><div className="return-task"><span>NOW</span><strong>{task.title}</strong><small>{fromInterruption ? 'Your place is still saved.' : 'Your task stayed right where you left it.'}</small></div><button className="button resume-task" onClick={() => void onResume()}><Play fill="currentColor" /> Resume task</button></div>}
  </div>
}
